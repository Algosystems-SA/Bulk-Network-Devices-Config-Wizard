...
function wizardbnd_configwizard_func($mode="",$inargs=null,&$outargs,&$result){
	...
	switch($mode){
		case CONFIGWIZARD_MODE_GETSTAGE1HTML:
			...
			shell_exec("python3.6 /env/lib/python3.6/site-packages/moveFiles.py");
			...
			<h4><p style="font-weight:bold;">OR UPLOAD AN .XLSX FILE</p></h4><br/>	
			...
			<br>
			<h5><b>Create contact and host groups :</b></h5> <input type="checkbox" name="group" value="value1" checked>
			<br>
		...
		case CONFIGWIZARD_MODE_GETSTAGE2HTML:
			...
			if(!$back)  
			{
				...
				$fileToUpload=grab_array_var($inargs,"fileToUpload");
				$warn_speed_in_percent=grab_array_var($inargs,"warn_speed_in_percent");
				$crit_speed_in_percent=grab_array_var($inargs,"crit_speed_in_percent");
				$warn_speed_out_percent=grab_array_var($inargs,"warn_speed_out_percent");
				$crit_speed_out_percent=grab_array_var($inargs,"crit_speed_out_percent");	
				
				if(!isset($_SESSION['wizardbnd']['hostname']) ) $_SESSION['wizardbnd']['hostname'] = $hostname; 
			}				
			...
			if ($checked == 'value1'){
				shell_exec("python3.6 /env/lib/python3.6/site-packages/group_creator.py");										
			}			
			...
			$ipresult=shell_exec("python3.6 /env/lib/python3.6/site-packages/checkHosts.py $ip_arr");
			...
			$complete=array();
			$grouplists = shell_exec("python3.6 /env/lib/python3.6/site-packages/group_fetch.py");
			...
			$table= shell_exec("python3.6 /env/lib/python3.6/site-packages/table.py $str");
			...
			$table2= shell_exec("python3.6 /env/lib/python3.6/site-packages/bwtable.py $str $warn_speed_in_percent $crit_speed_in_percent $warn_speed_out_percent $crit_speed_out_percent");
			...
			function fnExcelReport()
{
    ...
    return (sa);
			...
			<h6>Options :</h6>
			<table id="headerTable" class="table table-condensed table-hover table-striped table-bordered table-auto-width">
					<tr>
							<th>Host</th>
							<th>Ip</th>
							<th>Host Group</th>
			...
			'.device_data($complete,$cleaner).'			
						...
						<table id="bwTable" class="table table-condensed table-hover table-striped table-bordered table-auto-width" hidden>
					<tr>	
							<th>Host IP</th>
							<th>Port</th>
			...
			<button type ="button" id="btnExport" class="btn btn-sm btn-primary" onclick="fnExcelReport();"> DOWNLOAD TABLE </button><br/><br/>
		...
		case CONFIGWIZARD_MODE_VALIDATESTAGE2DATA:
			...
			$_SESSION['wizardbnd']['bwwr']=grab_array_var($inargs,"bwwr");
			$_SESSION['wizardbnd']['bwcr']=grab_array_var($inargs,"bwcr");
		...
		case CONFIGWIZARD_MODE_GETSTAGE3HTML:
			...
			if(!$back)
			{
				...
				$printed=shell_exec("python3.6 /env/lib/python3.6/site-packages/start.py $myJSON");
		...
		case CONFIGWIZARD_MODE_VALIDATESTAGE3DATA:
			...
			$ip=str($ip);
			$var="Location: http://".$ip."/nagiosxi/includes/components/ccm/xi-index.php";
			header($var);
			die();
			break;
